...
#define KRL_SIG_NAMESPACE	"SSHKRL"
...
int ssh_krl_revoked_certs_for_ca_key(struct ssh_krl *krl,
...
int ssh_krl_revoke_certs(struct ssh_krl *krl,
    const struct sshkey *ca_key, struct revoked_certs *certs);
...
int ssh_krl_to_file(const char *path, struct ssh_krl *krlp);
int ssh_krl_from_file(const char *path, struct ssh_krl **krlp);
